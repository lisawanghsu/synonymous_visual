...
if method == 'CS':
    cs_data = pd.read_csv(r'training-data/cscape_coding_training_examples.txt', sep='\t')
    st.write('CS training data:', cs_data.shape, cs_data)
elif method == 'EPEL':
    EPEL_data = pd.read_csv(r'training-data/EPEL_train_close7.txt', sep='\t')
    st.write('EPEL training data:', EPEL_data.shape, EPEL_data)
elif method == 'epSMic':
    epSMic_data = pd.read_csv(r'training-data/epSMic_training.vcf', sep='\t')
    st.write('epSMic training data:', epSMic_data.shape, epSMic_data)
elif method == 'MFDSMC':
    MFDSMC_data = pd.read_csv(r'training-data/MFDSMC_training.vcf', sep='\t', header=None)
    st.write('MFDSMC training data:', MFDSMC_data.shape, MFDSMC_data)
elif method == 'PredDSMC':
    PredDSMC_data = pd.read_csv(r'training-data/MFDSMC_training.vcf', sep='\t', header=None)
